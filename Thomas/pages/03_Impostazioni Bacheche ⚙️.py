...
from functions import check_password, autenticazione
...
if autenticazione():
    if check_password():
        ...
        choice = st.selectbox('Scegli che azione eseguire', ['','Crea/Elimina Bacheca', 'Crea task'])

        if choice == '':
            st.info('Puoi scegliere se creare/eliminare una tabella, aggiungere una task a una tabella', icon="ℹ️")
        ...
        if choice == 'Crea task':
            ...
            task = st.text_input("Aggiungi una task")
            ...
            if st.button("Aggiungi bacheca"):
                ...
                if task == '':
                    ...
                    db.collection(bacheca + '-da_fare').document(task).set({ 
                                'descrizione': str(descrizione),
                                })
                    #db.collection(bacheca + '-in_esecuzione').document(task).set({
                                                                            
                                #})
                    #db.collection(bacheca + '-fatto').document(task).set({ 
